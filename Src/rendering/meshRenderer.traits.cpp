...
std::vector<uint8_t> ComponentTraits<MeshRenderer>::serialize(
        Registry &registry,
        ...
        if (vertexCount > 0) {
                ...
                append(renderer->vertices.data(), vertexBytes);
        }
        ...
        if (indexCount > 0) {
                ...
                append(renderer->indices.data(), indexBytes);
        }
...
std::any ComponentTraits<MeshRenderer>::deserialize(
        ...
        if (vertexCount > 0) {
                ...
                read(vertices.data(), vertexBytes);
        }
        ...
        if (indexCount > 0) {
                const auto indexBytes =
                                static_cast<size_t>(indexCount) * sizeof(std::uint32_t);

                read(indices.data(), indexBytes);
        }
